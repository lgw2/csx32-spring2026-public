...
\\
\LARGE\textbf{Homework 4 Alternate}%
\\[0.5ex]
\large Due ?
\end{center}
...
\headers{CSCI 432/532}{Homework 4 alt}{Spring 2026}
...
%----------------------------------------------------------------------
\item
For any string $w \in \Set{\Sym0, \Sym1}^*$, let $dash(w)$ be the string in $\Set{\Sym0, \Sym1, \Sym{-}}^*$
obtained from $w$ by inserting a new symbol \Sym{-} between any two consecutive
appearances of the same symbol.
For example:
\begin{align*}
dash(\e) & = \e \\
dash(\Sym{0000}) & = \Sym{0-0-0-0} \\
dash(\Sym{10101}) & = \Sym{10101} \\
dash(\Sym{0010101110}) & = \Sym{0-010101-1-10}
\end{align*}
\begin{enumerate}
\item Let L be an arbitrary regular language over the alphabet $\Set{\Sym0, \Sym1}$. Prove that the
language $\mathsc{Dash}(L) = \{dash(w) : w \in L\}$ is also regular.
\item Let L be an arbitrary regular language over the alphabet $\Set{\Sym0, \Sym1}$. Prove that the
language $\mathsc{Undash}(L) = \{w : dash(w) \in L\}$ is also regular.
\end{enumerate}
...
%----------------------------------------------------------------------
\item
For each of the following languages over the alphabet $\Sigma =
\set{\Sym0,\Sym1}$, either prove that the language is regular (by constructing
an appropriate DFA, NFA, or regular expression) or prove that the language is
not regular (by constructing an infinite fooling set).  Recall that~$\Sigma^+$
denotes the set of all \emph{nonempty} strings over $\Sigma$.  Also recall that a
palindrome is a string that equals its own reversal.

\begin{enumerate}
\item
$\Set{xy : |x| \leq 67 \text{ and } |y| \leq 67 \text{ and } y \text{ is a palindrome}}$

\item
$\Set{xy : |x| \leq 67 \text{ and } |y| \leq 67 \text{ and } x \text{ is a palindrome}}$

\item
$\Set{wxw^R}{x, w\in \Sigma^+}$

\item
$\Set{xww^R}{x, w\in \Sigma^+}$

\end{enumerate}
\Hint{Exactly two of these languages are regular.}
\begin{Rubric}{Rubric}
    See rubrics for DFAs/NFAs, regular expressions, and fooling sets from prevoius homeworks.
\end{Rubric}
